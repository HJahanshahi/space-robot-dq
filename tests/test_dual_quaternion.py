...
class TestDualQuaternionBasics:
    def test_identity_matrix(self):
        np.testing.assert_allclose(DualQuaternion().to_matrix(), np.eye(4), atol=1e-10)

    def test_pure_rotation_roundtrip(self):
        R = Rotation.from_rotvec([0, 0, np.pi / 4]).as_matrix()
        dq = DualQuaternion.from_pose(R, np.zeros(3))
        R_out, p_out = dq.to_pose()
        np.testing.assert_allclose(R, R_out, atol=1e-10)
        np.testing.assert_allclose(p_out, np.zeros(3), atol=1e-10)

    def test_pure_translation_roundtrip(self):
        p = np.array([1.0, 2.0, 3.0])
        dq = DualQuaternion.from_pose(np.eye(3), p)
        _, p_out = dq.to_pose()
    ...
    def test_conjugate_is_inverse(self):
        ...
        np.testing.assert_allclose((dq * dq.conjugate()).to_matrix(), np.eye(4), atol=1e-8)
...
class TestLogDQ:
    def test_identity_gives_zero(self):
        np.testing.assert_allclose(log_dq(DualQuaternion()), np.zeros(6), atol=1e-8)

    def test_pure_translation(self):
        xi = log_dq(DualQuaternion.from_pose(np.eye(3), [0.5, 0, 0]))
        np.testing.assert_allclose(xi[:3], np.zeros(3), atol=1e-6)

    def test_pure_rotation(self):
        R = Rotation.from_rotvec([0, 0, 0.5]).as_matrix()
        xi = log_dq(DualQuaternion.from_pose(R, [0, 0, 0]))
        np.testing.assert_allclose(xi[3:], np.zeros(3), atol=1e-6)
